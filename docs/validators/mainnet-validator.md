...
Start by consulting the latest [Peggo release notes](https://github.com/umee-network/peggo/releases), to make sure you are aware about the update instructions.

1.  Download Peggo: the gravity bridge orchestrator [latest release for your architecture](https://github.com/umee-network/peggo/releases). You can build from source.
...
6.  Check the sync status. If catching_up is equal to true, wait until it is false. This process may take a minute.

        umeed status 2>&1 | jq -r '.SyncInfo.catching_up'

7.  Check to see if you have received tokens from the faucet or that you have tokens in your new Umee wallet.

        umeed q bank balances $(umeed keys show UMEE_WALLET_NAME -a)

8.  If the node was synced and you have tokens you can now create a validator using the following commands
...
9.  Link all keys on the bridge
    ...
    Also, if you got a "signing validation error" then repeat last 3 commands.

10. After a successful transaction from step 13 — run the orchestrator.
